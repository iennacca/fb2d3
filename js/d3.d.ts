...
interface ID3Layout {
    ...
    force(): ID3ForceLayout;
}

interface ID3ForceLayout {
    charge(chargeStrength: number): ID3ForceLayout;
    linkDistance(distance: number): ID3ForceLayout;
    size(dimensions: number[]): ID3ForceLayout;
    nodes(nodeTree: any[]): ID3ForceLayout;
    start(): void;
    drag(selection: ID3Selection): ID3Selection;
...
interface ID3PathGenerator {
    projection(projection: ID3Projection): ID3PathGenerator;
}
